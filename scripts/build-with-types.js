...
const containedFile = 'src/contained/FilesystemLibrary.ts';
const primaryFile = 'src/primary/FilesystemLibrary.ts';
...
try {
  ...
  const containedModified = containedOriginal
    .replace(
      /> extends AbstractFilesystemLibrary<V, S, L1, L2, L3, L4, L5> \{\s*operations: Contained\.Operations<V, S, L1, L2, L3, L4, L5>;\s*\}/,
  ...
  const primaryModified = primaryOriginal
    .replace(
      /> extends AbstractFilesystemLibrary<V, S> \{\s*operations: Primary\.Operations<V, S>;\s*\}/,
      `> {\n  operations: any;\n  coordinate: any;\n  options: any;\n  globalDirectory: string;\n  registry: any;\n}`
    );
